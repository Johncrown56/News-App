...
class SuperTabsContainerComponent {
    ...
    moveContainer(scrollX, animate) {
        if (animate) {
            utils.scrollEl(this.el, scrollX, 0, this.config.nativeSmoothScroll, this.config.transitionDuration);
        }
        else {
    ...
    async scrollToTop() {
        ...
        if (el) {
            utils.scrollEl(el, 0, 0, this.config.nativeSmoothScroll, this.config.transitionDuration);
        }
        else {
    ...
    async onTouchMove(ev) {
        ...
        const coords = utils.pointerCoord(ev);
        if (!this.isDragging) {
            if (!utils.checkGesture(coords, this.initialCoords, this.config)) {
